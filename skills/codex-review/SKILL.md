...
既定のレビュー対象は未コミットの変更である。ツリーをきれいにする操作は、レビュー対象そのものを消す。

このスキルの実行中、`git stash` / `git reset` / `git checkout` / `git switch` / `git restore` / `git clean` / `gh pr checkout` を実行してはならない。Phase 3 の修正だけが、ワークツリーへの書き込みである。

## Phase 1: Codex の実行

レビュー対象に応じてコマンドを選ぶ。リポジトリのルートで実行する。
...
`$ARGUMENTS` が `--base` や `--commit` を含む場合はそれに従う。無ければ `--uncommitted` を使う。
...
**修正するのは、ACCEPT かつ `P0` または `P1` の指摘だけである。**
...
`P2` 以下と DEFER は修正しない。報告に載せるだけである。ついでの整理もしない。
...
対象: <レビューした範囲>
Codex の指摘: <件数>件（ACCEPT <n> / REJECT <n> / DEFER <n>）
