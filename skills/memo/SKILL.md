...
style / strength / weakness はこの仕組みの主目的（自己の振り返り）なので、**セッション中にユーザーの傾向・良かった行動・つまずきを観測したら積極的に記録する**。遠慮して漏らすより、多めに拾う方がよい。特に strength は意識しないと拾い漏れやすいので、「今日ユーザーの判断で上手くいったことは何か」を必ず自問する。weakness も同様に拾い漏れやすい。「今日ユーザーが立ち止まった・確認を要した・想定と違った箇所はどこか」を必ず自問する。優秀なユーザーほど失敗が表面化する前に回復するため、**回復の速さに惑わされず、つまずきの発生自体を記録する**。お世辞ではなく、根拠のある事実だけを書く。weakness も内容は事実ベースで書き、人格評価はしない。

weakness の観測トリガー（セッション中にこれが起きたら inbox へ1行書く）:

- 同じ概念について2回以上言い換えの質問があった（1回の質問は健全な確認、繰り返しは理解の引っかかり）
- AI の説明に対して「つまりこういうこと？」という要約確認が誤っていた
- コマンド・コードの提示後に「これ何をしてる？」と目的の説明を求められた
- 一度説明した内容について後のターンで再度質問があった
...
1. `~/dev/knowledge/inbox.md` があれば読み、未処理行をトピック候補に含める。
   ...
   knowledge / decision だけでなく、「ユーザーは今日どこでつまずいたか」「どこで立ち止まった・確認を要したか」「AI の説明を initially 誤解した箇所はなかったか」「どんな判断の傾向を見せたか」「どの判断・行動が良かったか」を必ず自問する
