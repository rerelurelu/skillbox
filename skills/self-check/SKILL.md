...
これはレビューではない。答える問いは「第三者として、この実装に問題はないか」ではなく、**「実装者としてやるべきことを終えたか」**である。correctness・security・architecture・performance の掘り下げは `codex-review` の担当であり、ここではやらない。両方で同じことをすると、レビューを 2 セット行うだけになる。

## subagent を使わない

観点別の subagent を起動してはならない。起動した時点でセルフチェックではなくレビューになり、`codex-review` と重複する。main agent 自身が diff を読み返す。
...
要件との突合:
- <各項目と実装の対応、または未実装項目とその理由>

diff の読み返し:
- <見つけて直した内容、または問題なし>

残骸:
- <削除したもの、意図して残したものとその理由、または無し>
