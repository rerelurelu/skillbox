...
**この手順はレビュアー自身が、レビュー範囲を読んだあとで実行する。** main agent が代わりに調べて渡すことはしない。どのライブラリが version-sensitive かは、変更ファイルを実際に読んだレビュアーの方が正確に判断できる。
...
役割別レビュアーは `Bash` を持つが、`node -p`・`npm view`・`curl` のようなネットワークを使うコマンドは、持っていても使わない。レビューのたびに外部のレジストリへ問い合わせると、結果が実行のたびに変わって再現性が下がり、ネットワークの無い環境でレビューが動かなくなる。**そのため、以下の手順はすべてローカルのファイルを `Read` で直接読む方法だけで組み立ててある。** ローカルにキャッシュされたファイルが無ければ、そのライブラリは諦めてスキップする（推測や記憶で埋めない）。
...
ローカルにキャッシュが無い、依存がインストールされていない、対応表に載っていないエコシステムのいずれでも、そのライブラリは質問対象から外す。推測でリポジトリ名を組み立てない。`npm view` や `curl` でレジストリに問い合わせて解決しようとしない。
...
**main agent がトリアージするとき**: バージョン整合性に関する指摘は、deepwiki の回答を証拠にして採否を決める。
