...
#                  呼び出し側がファイル作成ツールで <tmpdir>/request.md を書いてから渡す。
...
#   `rc` を使う。
# - `codex exec resume <session-id>` は使わず、毎回ステートレスな `codex exec` を使う。
#   実機検証で、resume した 2 回目の呼び出しが 1 回目の `--sandbox read-only` を
#   引き継がず、ワークツリーへの書き込みが成功した。resume がサンドボックスモードを
#   安全に継承すると公式に確認できない限り、この方式は変えない。
