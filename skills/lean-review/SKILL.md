...
description: |
  ...
  Use when: the user explicitly asks for a lean/simplicity review of code that implementation and codex-review are already done on.
user-invocable: true
argument-hint: "[scope]"
license: "GPL-3.0"
---
...
Memorable adapter for the `ponytail-review` skill. Use once implementation and the independent correctness review (`codex-review`) are done and the code is expected to be final.
...
- Don't assign or convert severity, and don't map findings onto categories from other review skills (e.g. codex-review's P0-P3 or "must fix / consider").
...
- Don't check whether `codex-review` has already run or otherwise gate on prior review state — run whenever the user explicitly invokes this skill.
- Don't invoke or depend on `codex-review` in either direction; the two skills stay independent.
