...
| [codex-review](skills/codex-review/SKILL.md) | Runs `codex review` for an independent read-only review, then the main agent checks each finding itself and decides accept / reject / defer. Fixes the accepted P0 and P1 findings and reports the rest, including anything that needs a decision (needs the `codex` CLI) |
